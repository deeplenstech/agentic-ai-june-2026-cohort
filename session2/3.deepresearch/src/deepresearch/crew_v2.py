...
def make_researcher_agent() -> Agent:
    # Each parallel (async_execution) research task needs its own agent instance.
    # A shared agent holds a single mutable message history, and concurrent tool
    # loops would interleave their tool_use/tool_result blocks — corrupting the
    # conversation and failing Bedrock's toolResult-ID validation.
    return Agent(
